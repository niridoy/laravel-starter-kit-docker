...
@section('title', 'Bondstein || Slot Edit')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="ibox">
                ...
                <div class="ibox-body">
                    <form class="form-horizontal" action="{{ route('slots.update', $slot) }}" method="POST">
                            @csrf
                            @method('PUT')
                        <div class="row">
                                ...
                                <div class="col-sm-8">
                                    <select name="project_id" class="form-control select_item_list" id="terminalId">
                                        ...
                                        @foreach ($projects as $project)
                                            <option value="{{ $project->id }}"  {{ $project->id == old('project_id', $slot->project_id) ? 'selected' : '' }} >{{ $project->name }} ({{ $project->code }})</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
